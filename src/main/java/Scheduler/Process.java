...
public class Process implements Cloneable {
  ...
  private int PID, arrivalTime, burstTime, startTime,
          completionTime, turnaroundTime, waitTime,
          responseTime, remainingBurstTime, coreNeeded;
  ...
  public Process() {
    ...
    this.setStartTime(-1);
    this.setCoreNeeded(1);
  }

  public Process(int PID, int arrivalTime, int burstTime, int coreNeeded) {
    this.setPID(PID);
    this.setArrivalTime(arrivalTime);
    this.setBurstTime(burstTime);
    this.setRemainingBurstTime(burstTime);
    this.setStartTime(-1);
    this.setCoreNeeded(coreNeeded);
  }
  ...
  */
  ...
  public String toString() {
    String processInfo = String.format("[PID %d] | Arrival Time %d | Burst Time: %d | Remaining Burst Time: %d | Start Time: %d | Completion Time: %d | Turnaround Time: %d | Wait Time: %d | Response Time: %d | Core Needed: %d",
      this.getPID(), this.getArrivalTime(), this.getBurstTime(), this.getRemainingBurstTime(), this.getStartTime(), this.getCompletionTime(), this.getTurnaroundTime(), this.getWaitTime(), this.getResponseTime(), this.getCoreNeeded()
    );
    return processInfo;
  }
