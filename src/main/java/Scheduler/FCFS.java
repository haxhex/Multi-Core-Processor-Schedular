...
public class FCFS {
  ...
  private void printSummary(ArrayList<Process> jobs) {
    System.out.println("\n>>> Scheduling Summary <<<");
    System.out.printf("+%s+\n", "-".repeat(114));
    System.out.printf("|%2sProcess ID%2s|%2sArrival Time%2s|%2sBurst Time%2s|%2sTurnaround Time%2s|%2sWait Time%2s|%2sResponse Time%2s|%2sCore Needed%2s|\n",
      "", "", "", "", "", "", "", "", "", "", "", "", "", "");
    System.out.printf("+%s+\n", "-".repeat(114));

    for (Process process : jobs) {
      System.out.printf("|%6s%02d%6s|%7s%02d%7s|%6s%02d%6s|%9s%02d%8s|%6s%02d%5s|%8s%02d%7s|%7s%02d%6s|\n",
        "", process.getPID(), "",
